...
lcn2_plt_out <- processDF(df=lcn2_df1, calc_key=lcn2_calc_key, start_idx=lcn2_start_idx, plotallagg=plot_all_agg,
                          show_p_correct=correctp, labelsize=20, ptsize=1.5, rm_outliers=F, makemainfig=T, plot_GT_separate=T)
...
if (!is.null(lcn2_df) & 'section' %in% names(lcn2_df)) {
  ...
  print(paste("Sex:", paste(unique(sex_analyze), collapse=", ")))
  sectiondf1 <- lcn2_df %>% 
    group_by(id, slice) %>% 
...
```{r plot_facet_lcn2, echo=FALSE, fig.height=15, fig.width=25, warning=FALSE, message=FALSE, results='hide', eval=shownewplot}
if (!is.null(lcn2_df)) {
  # good dims: 6x17
  # new good dims: height=27, width=17
  # note: dimensions of plots are designed to view after knitting
 #  fig.height=20, fig.width=19
  # for GT separate:
  #  fig.height=15, fig.width=19
